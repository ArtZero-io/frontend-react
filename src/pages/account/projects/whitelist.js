...
function MyWhiteListProjectPage() {
  ...
  const onGetOwner = async (e) => {
    ...
    if (projectCount > 0) {
      for (let i = 1; i <= projectCount; i++) {
        const projectAddress = await launchpad_contract_calls.getProjectById(
          currentAccount,
          i
        );
        ...
        const contractAdminAddressawait =
  ...
  const onChangePhaseCode = async (phaseId) => {
    ...
    if (phaseId && projectAddress) {
      ...
      console.log(
        "MyWhiteListProjectPage::totalPhaseAccountLink",
        totalPhaseAccountLink
      );
      for (let i = 1; i <= totalPhaseAccountLink; i++) {
        ...
        if (whiteListData) {
          ...
          console.log(
            "MyWhiteListProjectPage::whiteListDataItemTmp",
            whiteListDataItemTmp
          );
                  ...
                  >
                    <Box h="full">
                      ...
                      <Box h="full">
                        {" "}
                        <Box mt={7}>
                          <Text py={2}>Chose Project</Text>
                          <Box>
                            <Select
                              placeholder="Select project"
                              onChange={(event) =>
                                updateProjectAddress(event.target.value)
                              }
                            >
