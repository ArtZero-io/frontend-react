...
function MyAzeroDomainsNFTCard({
  nftContractAddress,
  is_for_sale,
  is_locked,
  price,
  ...
  const handleOnChangeCheckbox = ({ target }) => {
    if (target.checked && multiStakeData?.list?.length >= MAX_ITEM_STAKE) {
      !multiStakeData?.list?.includes(azDomainName) && setIsTicked(false);
  ...
  const handleOnChangeMultiDelistCheckbox = (e) => {
    ...
    if (
      ...
      handleSelectMultiDelist(azDomainName ?? tokenID, "MULTI_DELIST", target);
      return;
    }
  ...
  const handleOnChangeMultiListCheckbox = (e) => {
    ...
    const target = !multiListingData?.list?.includes(azDomainName);
    if (target && multiListingData?.list?.length >= MAX_ITEM_BULK_LISTING) {
      !multiListingData?.list?.includes(azDomainName) &&
    ...
    if (
      ...
      handleSelectMultiListing(azDomainName, "MULTI_LISTING", target);
      return;
    }
  ...
  const handleOnChangeMultiTransferCheckbox = (e) => {
    ...
    const target = !multiTransferData?.list?.includes(azDomainName);

    if (target && multiTransferData?.list?.length >= MAX_ITEM_BULK_TRANSFER) {
      !multiTransferData?.list?.includes(azDomainName) &&
    ...
    <Flex className="my-nft-card-wrapper">
      <motion.div
        ...
        style={{
          ...
          borderWidth: "2px",
          borderColor: `${
            multiStakeData?.list?.includes(azDomainName)
              ? "#7ae7ff"
              : "transparent"
          }`,
          maxWidth: cardSize,
      ...
      >
        ...
        >
          ...
          <Box w="full" p={3}>
            ...
            {!is_for_sale && stakeStatus !== 0 ? (
              <Flex align="center" justify="start" w="full">
                <CommonButton
                  ...
                  isDisabled={
                    actionType && !tokenIDArray?.includes(azDomainName)
                      ? true
                      : multiStakeData?.action
                      ? true
                      : false
            ...
            {expiration_timestamp < Date.now() && <>This domain is expired</>}
...
export default MyAzeroDomainsNFTCard;
