  ...
  fs.readFile(indexPath, "utf8", async (err, htmlData) => {
    try {
      ...
      if (req.params?.[0]) {
        ...
        htmlData = htmlData
          .replaceAll(process.env.REACT_APP_WEBSITE_TITLE, project.name)
          .replaceAll(
            process.env.REACT_APP_WEBSITE_DESCRIPTION,
            project.description
          )
          .replaceAll(process.env.REACT_APP_WEBSITE_IMAGE, image);
      }
  ...
  fs.readFile(indexPath, "utf8", async (err, htmlData) => {
    try {
      ...
      if (req.params?.[0]) {
        ...
        htmlData = htmlData
          .replaceAll(process.env.REACT_APP_WEBSITE_TITLE, project.nftName)
          .replaceAll(
            process.env.REACT_APP_WEBSITE_DESCRIPTION,
            project.description
          )
