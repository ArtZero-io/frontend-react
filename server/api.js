...
exports.getCollectionByAddress = async ({ collection_address }) => {
  const ret = await clientPost(`/getCollectionByAddress`, {
    collection_address,
  });
  // TEMP format to match current return format
  return ret.ret;
...
exports.getNFTByID = async ({ collection_address, token_id }) => {
  const ret = await clientPost(`/getNFTByID`, { collection_address, token_id });
  // TEMP format to match current return format
  return ret.ret;
};
